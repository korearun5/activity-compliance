...
| CARBON-UAT-04 | Add/view farmer enrollment       | Carbon identity ID, farmer name, village/taluka, landholding, tillage, bank status, and document status are visible through the Carbon "Add farmer" enrollment flow. | Ready   |
| CARBON-UAT-05 | Review farm/GPS data             | Farm area and GPS point are captured; map boundary drawing is not required.                                                                                          | Pending |
| CARBON-UAT-06 | Review soil profile values       | SOC, pH, EC, N, P, K, bulk density, and texture are accepted as stored data only.                                                                                    | Ready   |
| CARBON-UAT-07 | Upload or link soil report       | PDF/image upload stores file metadata through the storage adapter without triggering carbon calculation.                                                             | Ready   |
...
| CARBON-UAT-09 | Create activity record           | Activity entry stores farm, crop, category, date, input, quantity, remarks, evidence count, and verification status.                                                 | Ready   |
...
Current Carbon durable schema has backend APIs for profile, plot, soil, soil
report upload, activity categories, and activity records. Validate through
Flyway/integration startup and API-level UAT checks.
