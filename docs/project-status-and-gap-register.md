...
| Module subscription platform |        92% | Medium     | Backend guards, central frontend module visibility registry, Carbon/FPO client package toggles, Carbon module extraction, and module visibility smoke coverage exist; handover packaging remains pending.                                                                                                                                                                                                                                                    |
...
| Carbon API implementation                   | Done        | Backend/QA          | Backend APIs for Carbon profiles, farm plots, soil profile metadata, soil report upload, activity categories, and activity records are implemented with role/module scoping and focused Testcontainers coverage.                                           |
| Carbon profile frontend wiring              | Done        | Frontend/QA         | Carbon admin/FPO/field-coordinator screens can list/create/update farmer enrollments, plots, soil metadata, and activity records; farmer Carbon screen reads the linked backend profile, plots, soil data, and activity data.                              |
| Carbon farmer account linking               | Pending     | Frontend/Backend    | Carbon enrollment can store `userId`/`fpoMemberProfileId`, but the staff UI still needs a friendly farmer account/member picker or create-login flow before client demo polish.                                                                            |
