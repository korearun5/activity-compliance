...
| Frontend platform shell      |        78% | Medium     | Backend-first admin and FIELD_COORDINATOR shell exists; frontend still lacks automated UI tests. |
| Auth, users, and roles       |        85% | High       | JWT, role APIs, local seeds, frontend routing, and tests now use `ADMIN`, `FPO_MANAGER`, and `FIELD_COORDINATOR`. |
...
| FPO member management        |        86% | High       | Approved farmer fields now align across schema, API, admin UI, validation, and focused tests; FPO ownership isolation remains separate. |
...
- FPO MVP technical foundation: about 81%.
- FPO Phase 1 go-live readiness before UAT: about 80-83% after client scope lock; implementation gaps remain.
...
- Add browser/E2E smoke tests for login, admin FPO flows, and FIELD_COORDINATOR flows.
...
| Role foundation alignment | Done | Backend/Frontend/QA | Active app code now uses `ADMIN`, `FPO_MANAGER`, and `FIELD_COORDINATOR`; focused role/user/member tests pass. |
...
| FPO ownership/scoped access alignment | Pending | Backend/QA | FPO ownership/scoping remains to be implemented; farmer login linkage is intentionally retained for Phase 2. |
...
2. Add FPO ownership/scoped access tests without removing the member-user link
   preserved for Phase 2 farmer login.
3. Rehearse the clean-start runbook on a fresh machine or clean Windows profile.
4. Finalize the client-facing operations manual after production hosting is
   chosen.
5. Convert the UAT guide into automated smoke/integration coverage where useful.
6. Decide whether to enforce Jacoco coverage thresholds in CI.
7. Add frontend test tooling before the UI grows further.
8. Add storage integration coverage for MinIO/S3-compatible flows.
